...
export async function GET(request: NextRequest) {
  ...
  const allChecks24h = await prisma.endpointHealth.findMany({
    where: {
      endpointId: { in: endpoints.map((ep) => ep.id) },
      timestamp: { gte: twentyFourHoursAgo },
    },
  });

  const checksByEndpoint = new Map<string, typeof allChecks24h>();
  for (const check of allChecks24h) {
    const arr = checksByEndpoint.get(check.endpointId);
    if (arr) arr.push(check);
    else checksByEndpoint.set(check.endpointId, [check]);
  }

  const items = endpoints.map((ep) => {
    const checks24h = checksByEndpoint.get(ep.id) ?? [];
